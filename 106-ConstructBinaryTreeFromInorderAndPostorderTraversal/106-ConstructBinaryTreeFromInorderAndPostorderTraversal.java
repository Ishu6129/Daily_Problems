// Last updated: 24/08/2025, 22:49:29
/**
...
class Solution {
    ...
    public TreeNode createTree(int[] in,int[] pos,int ilo,int ihi,int plo,int phi){
        if(ilo>ihi || plo>phi) return null;
        TreeNode node=new TreeNode(pos[phi]);
        int idx=search(in,ilo,ihi,pos[phi]);
        int c=idx-ilo;
        node.left=createTree(in,pos,ilo,idx-1,plo,plo+c-1);
        node.right=createTree(in,pos,idx+1,ihi,plo+c,phi-1);
        return node;
    }
